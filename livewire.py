...
class Livewire():
    """
    A simple Intelligent scissors implementation for verification using 
    ...
    def __init__(self, image):
        ...
        self.gradient = np.empty([self.x_lim, self.y_lim], dtype=object)
        ...
        for i in range(1, height-1):
            for j in range(1, width-1):
                ...
                for bors in neighbors:
                    ...
                    if diagonal:
                        gradient[i,j][(bors[0], bors[1])] = math.fabs(image[i+1, j] - image[i, j-1])/math.sqrt(2)
                        maxval = max(maxval, gradient[i,j][bors])

                    else:
                        gradient[i,j][(bors[0], bors[1])] = math.fabs(image[i, j-1] + image[i+1, j-1] - image[i, j+1] - image[i+1, j+1])/4
                        maxval = max(maxval, gradient[i,j][bors])
